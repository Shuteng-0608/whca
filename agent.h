...
private:
  ...
  std::vector<std::vector<std::pair<int, int>>> coordinates; // 三维数组
...
public:
  ...
  void setCoordinates(const std::vector<std::vector<std::pair<int, int>>>& coords); // 设置坐标的函数声明
  std::vector<std::vector<std::pair<int, int>>> getCoordinates() ; // 获取坐标的函数声明
  std::pair<int, std::vector<std::pair<int, int>>> getComposition(int w);
};
