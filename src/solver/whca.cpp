...
bool WHCA::solve() {
  solveStart();
  ROS_INFO("[WHCA] Solve time count start");
  ...
  // initialize path
  ROS_INFO("[WHCA] Initialize path");
  for (auto a : A) PATHS.push_back({ a->getNode() });
  ROS_INFO("[WHCA] Size of agent : %ld", A.size());
  int makespan = 0;
  while (!P->isSolved()) {
    for (int i = 0; i < A.size(); ++i) {
      ROS_INFO("[WHCA] Get path for agent %d", i);
    ...
    if (failed) {
      ROS_INFO("[WHCA] FAILED");
      break;
    }

    formalizePath(PATHS);
    ROS_INFO("[WHCA] Formalize Path");
  ...
  solveEnd();
  ROS_INFO("[WHCA] Solve time count end");
  ROS_INFO("[WHCA] Makespan : %d", makespan);
  return !failed;
}
