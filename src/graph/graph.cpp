...
Paths Graph::getFixedStartGoal(Nodes fixedStarts, Nodes fixedGoals) {
  
  if (fixedStarts.size() != fixedGoals.size()) {
    std::cout << "error@Graph::getFixedStartGoal, start and goal sizes don't match" << "\n";
    std::exit(1);
  }
  Paths points;
  for (auto i = 0; i < fixedGoals.size(); i++) {
    points.push_back({ fixedStarts[i], fixedGoals[i]});
  }
  return points;
}
