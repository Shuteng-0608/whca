...
void MAPF:: publishGoals (int makespan,int width){
  ...
  for (int i = 0; i < makespan; i++) {
    ...
    goal_0.target_pose.pose.position.x = composition_0.second[0].first;
    goal_0.target_pose.pose.position.y = composition_0.second[0].second;
    goal_0.target_pose.pose.orientation.w = 1;

    goal_1.target_pose.header.stamp = ros::Time::now();
    goal_1.target_pose.pose.position.x = composition_1.second[0].first;
    goal_1.target_pose.pose.position.y = composition_1.second[0].second;
